...
void setup() {
  ...
  while (!Serial) ;  // wait for Leonardo's serial port to connect.  needed for Leonardo only
  
  Serial.begin(9600); 
  geiger.begin(9600);

  Serial.println("New Geiger 1.0");
...
void loop() {
  ...
  {
    Serial.print ("Read:   "); Serial.println (geigerData);
  }
#if 0  
  else
  {
    Serial.println ("No Geiger Data yet,,,,,,");
  }
#endif
